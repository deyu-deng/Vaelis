"""ACP permission bridging for Vaelis dangerous-command approvals."""
...
# Maps ACP permission option ids to Vaelis approval result strings.
...
def _build_permission_options(*, allow_permanent: bool) -> list[PermissionOption]:
    """Return ACP options that match Vaelis approval semantics."""
    options = [
        PermissionOption(option_id="allow_once", kind="allow_once", name="Allow once"),
        PermissionOption(
            option_id="allow_session",
            # ACP has no session-scoped kind, so use the closest persistent
            # hint while keeping Vaelis semantics in the option id.
            kind="allow_always",
...
def _map_outcome_to_hermes(outcome: object, *, allowed_option_ids: set[str]) -> str:
    """Map an ACP permission outcome into Vaelis approval strings."""
...
def make_approval_callback(
    ...
    """
    Return a Vaelis-compatible approval callback that bridges to ACP.
