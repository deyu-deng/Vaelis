"""ACP auth helpers — detect and advertise Vaelis authentication methods."""
...
def detect_provider() -> Optional[str]:
    """Resolve the active Vaelis runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Vaelis can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Vaelis.
    ...
    not have Vaelis provider credentials configured yet, so Vaelis always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Vaelis using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure Vaelis provider",
            description=(
                "Open Vaelis' interactive model/provider setup in a terminal. "
                "Use this when Vaelis has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )
