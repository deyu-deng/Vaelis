...
row taxonomy is inferred from the atlas shape so Vaelis can render both legacy
...
# rows (1536x1872), while older Vaelis/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Vaelis' activity state names. They are not always identical to the
...
# Legacy Vaelis/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Vaelis activity names -> accepted row-name aliases in descending
