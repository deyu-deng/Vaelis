"""Upload a Vaelis session transcript to Hugging Face as an agent trace.

Vaelis stores sessions in its own SQLite store (``hermes_state.SessionDB``),
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through Vaelis' secret redactor (``force=True``) unless the
...
# ---------------------------------------------------------------------------
# Conversion: Vaelis OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses Vaelis' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render Vaelis conversation messages as Claude Code JSONL text.
