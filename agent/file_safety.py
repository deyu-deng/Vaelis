...
def _hermes_root_path() -> Path:
    """Resolve the Vaelis root dir (always the parent of any profile, never per-profile)."""
    try:
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Vaelis path.

    Three categories are blocked:

      * Internal Vaelis cache files under ``HERMES_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under HERMES_HOME and the global Vaelis
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    # Vaelis root so credential stores at <root>/auth.json etc. are also
    ...
    for hd in hermes_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal Vaelis cache file "
    ...
    for hd in hermes_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a Vaelis credential store "
    ...
    for hd in hermes_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the Vaelis MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a Vaelis MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
def raise_if_read_blocked(path: str) -> None:
    """Raise ``ValueError`` if ``path`` is a denied Vaelis read (see
...
#
# Vaelis profiles are separate HERMES_HOME dirs under
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Vaelis scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    Vaelis entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to Vaelis profile {info['target_profile']!r}, but the "
...
# regardless of which Vaelis profile is active. It does NOT cover the
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    ...
    index where the inner Vaelis-state portion starts. Returns ``None`` for
...
def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Vaelis state.
    ...
    Detection is path-shape-only — does not require any Vaelis resolver to
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host Vaelis process never "
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative Vaelis state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host Vaelis "
