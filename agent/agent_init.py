...
def init_agent(
    ...
    # WP-L1-EVERY-TURN: every turn spawns a fresh ``AIAgent`` on the desktop
    # gateway path; the ``on_session_start`` plugin hook only fires for the
    # very first turn. Re-apply the L1 default-profile budget knobs here so
    ...
    except Exception as exc:  # noqa: BLE001 - never break init_agent on plugin miss
        logger.debug("apply_l1_budget on init_agent path failed (non-fatal): %s", exc)
