...
Plugins built on Vaelis Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Vaelis
...
embedded slugs, no shorthands. This mirrors Vaelis' main config
...
handles every provider, fallback chain, and per-task override Vaelis
supports.
"""
