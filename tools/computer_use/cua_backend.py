...
def _cua_telemetry_disabled() -> bool:
    """True when Vaelis should disable cua-driver telemetry for this user.
    ...
    async def _lifecycle_coro(self) -> None:
        ...
        try:
            ...
            params = StdioServerParameters(
                ...
                # sanitize Vaelis-managed secrets out of the child env.
                env=_sanitize_subprocess_env(cua_driver_child_env()),
            )
...
class CuaDriverBackend(ComputerUseBackend):
    ...
    def __init__(self) -> None:
        ...
        #   - Gets a distinct agent-cursor color per Vaelis run, with
        ...
        #     concurrent Vaelis runs / subagents don't step on each
        #     other.
        # We mint a UUID4-based id once per CuaDriverBackend instance —
        # one Vaelis run = one backend = one session — and pass it as
    ...
    def start(self) -> None:
        ...
        # `computer-use` / `mcp` extras), not part of Vaelis' minimal core.
