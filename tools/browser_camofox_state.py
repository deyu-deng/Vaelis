"""Vaelis-managed Camofox state helpers.

Provides profile-scoped identity and state directory paths for Camofox
persistent browser profiles.  When managed persistence is enabled, Vaelis
...
def get_camofox_identity(task_id: Optional[str] = None) -> Dict[str, str]:
    """Return the stable Vaelis-managed Camofox identity for this profile.

    The user identity is profile-scoped (same Vaelis profile = same userId).
