...
<url>``) is spawned as a direct child of the Vaelis process. Vaelis's own
...
Vaelis process dies hard — ``kill -9``, an OS-level crash, a force-quit of
...
parent dies, so nothing reaps these until the next Vaelis startup's opt-in
