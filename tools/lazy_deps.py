"""
Lazy dependency installer for opt-in Vaelis Agent backends.

Many Vaelis features (Mistral TTS, ElevenLabs TTS, Honcho memory, Bedrock,
...
* **Durable-target mode (immutable images).** When the deployment seals the
  ...
  installed package cannot brick Vaelis, which is what made it safe to seal
...
def _unsupported_feature_reason(feature: str) -> Optional[str]:
    ...
    if sys.platform == "win32" and feature == "platform.matrix":
        return (
            ...
            "from sdist. Run Vaelis under WSL to use Matrix on Windows."
        )
    return None
