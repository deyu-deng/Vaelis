...
def _check_logging_callback_support() -> bool:
    ...
    server emits, so server-side diagnostics never reach Vaelis' logs.
    """
    if not _MCP_AVAILABLE:
        return False
    try:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            candidates = [
                ...
                # image (which the Vaelis Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → Vaelis MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that Vaelis gateways know how to render.
...
class ElicitationHandler:
    ...
    Form-mode elicitations are routed through Vaelis' existing approval
...
class MCPServerTask:
    ...
    def _make_logging_callback(self):
        ...
        server into Vaelis' logging (agent.log via hermes_logging), tagged
    ...
    async def _run_stdio(self, config: dict):
        ...
        # ungraceful exit of this Vaelis process (kill -9, crash, force-quit)
    ...
    async def run(self, config: dict):
        ...
        # routes those requests through Vaelis' approval system.
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until Vaelis is fully
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Vaelis config file.
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through Vaelis' MEDIA: tag convention and out to messaging
            ...
            # Vaelis' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Vaelis's historical behavior of converting hyphens to
...
# Native MCP tool-name prefix. Vaelis uses the ``mcp__<server>__<tool>``
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Vaelis registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Does NOT register tools in the Vaelis registry.
