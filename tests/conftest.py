...
# ── Mind writer: tests never create git commits ─────────────────────────────
#
# Why: ``MindWriter.__init__`` defaults to ``commit=True`` and ``_commit``
...
def _mind_writer_never_commits(monkeypatch):
    ...
    monkeypatch.setattr(
        MindWriter,
        "_commit",
        lambda self, root, paths: (True, "commit disabled in tests"),
    )
