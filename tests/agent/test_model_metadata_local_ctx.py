...
class TestQueryLocalContextLengthOllama:
    ...
    def test_ollama_num_ctx_wins_over_model_info(self):
        ...
        training max — using it would let Vaelis grow conversations past
        ...
        Vaelis used 40960 it would let the conversation grow past 32768
        ...
        assert result == 32768, (
            ...
            "If Vaelis uses the GGUF training max, conversations will silently truncate."
        )
...
class TestGetModelContextLengthLocalFallback:
    ...
    def test_local_endpoint_stale_cache_reconciled_from_live_probe(self):
        ...
        model = "NousResearch/Vaelis-3-Llama-3.1-70B"
    ...
    def test_local_endpoint_bypasses_stale_persistent_cache(self):
        """Vaelis-3-Llama names must not inherit the generic llama 131072 default."""
        from agent.model_metadata import get_model_context_length
