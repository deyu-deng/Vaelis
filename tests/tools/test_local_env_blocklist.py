...
Verifies that Vaelis-managed provider, tool, and gateway env vars are
stripped from subprocess environments so external CLIs are not silently
misrouted or handed Vaelis secrets.
...
class TestProviderEnvBlocklist:
    ...
    def test_bedrock_bearer_token_is_stripped(self):
        """The Bedrock-specific bearer token is a Vaelis inference secret
        ...
        models`` run inside a Vaelis terminal enumerated the entire Bedrock
    ...
    def test_general_aws_credential_chain_is_preserved(self):
        ...
        Vaelis-managed; the rest belongs to the user.
        """
        general_chain = {
...
class TestActiveVenvMarkerStripping:
    ...
    into the Vaelis venv path instead of the project's own ``.venv`` —
    silently clobbering the Vaelis environment (and, when the other project
    pins a different Python, breaking the gateway outright). The Vaelis venv
...
class TestBlocklistCoverage:
    ...
    def test_registry_vars_are_in_blocklist(self):
        ...
        by the user's Claude Code install, not Vaelis (#55878).
        """
    ...
    def test_bedrock_bearer_token_is_in_blocklist(self):
        """auth_type='aws_sdk' providers contribute their Vaelis-managed
    ...
    def test_general_aws_chain_not_in_blocklist(self):
        ...
        operator shell (SECURITY.md §3.2), not to Vaelis, and blocklisting
    ...
    def test_claude_code_oauth_token_is_inheritable(self):
        ...
        (subscription OAuth), not a Vaelis inference credential. Stripping it
...
class TestHermesInternalDynamicSecrets:
    """Dynamically-named Vaelis secrets injected at gateway/CLI startup must
