...
Vaelis drives [cua-driver](https://github.com/trycua/cua) under the hood
for the platform plumbing. The Vaelis-side `computer_use` tool exposed
in this skill is a higher-level Vaelis vocabulary; the raw cua-driver
...
Vaelis intentionally keeps THIS skill focused on the Vaelis-side
...
When `cua-driver skills install` autodetects Vaelis (planned follow-up
