# Vaelis Observer Hooks

Vaelis observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Vaelis' planner, model providers, memory, tool registry,
...
Hook callbacks are fail-open. Vaelis catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Vaelis only builds
