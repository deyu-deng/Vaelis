"""Shared constants for Vaelis Agent.
...
def set_hermes_home_override(path: str | Path | None) -> Token:
    """Set a context-local Vaelis home override and return its reset token.
...
def reset_hermes_home_override(token: Token) -> None:
    """Restore the previous context-local Vaelis home override."""
    _HERMES_HOME_OVERRIDE.reset(token)


def get_hermes_home_override() -> str | None:
    """Return the active context-local Vaelis home override, if any."""
...
def _get_platform_default_hermes_home() -> Path:
    """Return the platform-native default Vaelis home path."""
...
def get_hermes_home() -> Path:
    """Return the Vaelis home directory (default: platform-native path).
...
def get_default_hermes_root() -> Path:
    """Return the root Vaelis directory for profile-level operations.

    In standard deployments this is the platform-native Vaelis home
...
def _get_packaged_data_dir(name: str) -> Path | None:
    ...
    Used to discover bundled skills/optional-skills when Vaelis is installed
...
def get_hermes_dir(new_subpath: str, old_name: str) -> Path:
    """Resolve a Vaelis subdirectory with backward compatibility.
...
def iter_hermes_node_dirs(home: Path | None = None) -> list[Path]:
    """Return Vaelis-managed Node.js directories in preferred lookup order.
...
def node_tool_runnable(path: str | None) -> bool:
    ...
    Vaelis-managed Node trees live under ``$HERMES_HOME/node`` (or a profile's
...
def hermes_managed_node_tree_present(home: Path | None = None) -> bool:
    """Return True when any Vaelis-managed node/npm/npx shim exists on disk."""
    names = set()
...
def heal_hermes_managed_node() -> bool:
    """Redownload Vaelis-managed Node when the tree exists but is broken.
...
def find_hermes_node_executable(command: str) -> str | None:
    """Return a Vaelis-managed Node/npm executable path, healing broken trees."""
...
def find_node_executable_on_path(command: str) -> str | None:
    ...
    directly, so prefer the launchable variants explicitly for Vaelis-owned
    subprocesses.
    """
    if sys.platform != "win32":
...
def find_node_executable(command: str) -> str | None:
    """Resolve a Node.js command, preferring healthy Vaelis-managed installs.

    This is for Vaelis-owned subprocesses that should not be broken by a bad,
...
def with_hermes_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with Vaelis-managed Node directories prepended to PATH."""
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """Return the OS user's real home directory, avoiding Vaelis profile HOME.

    ``HERMES_HOME`` scopes Vaelis state. ``HOME`` is reserved for the OS/user
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply Vaelis' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
