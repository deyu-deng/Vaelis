...
How the agent executes each broker `optout.method` using native Vaelis tools. Obey **least-disclosure**:
...
1. **browser mode (no password, autonomous):** the command returns a recipient-locked `compose`
   ...
   stored. Requires the inbox signed in in the browser Vaelis uses.
...
- **Phase 1 (scan, read-only):** a cloud stealth browser (Browserbase) or the `scrapling` skill is
  ...
  `$HERMES_HOME/.env`, not just the shell env, so `doctor`/`setup --auto` detect the key Vaelis
...
- **Phase 2 (execute: opt-out forms, webmail sends, session-bound multi-step gates):** the work must
  ...
  binding).
- **How to drive the operator's browser (CDP).** Point Vaelis's browser tools at the operator's real
  ...
  fingerprint + logged-in sessions, which is precisely what Phase-2 flows need. (This is a Vaelis-side
