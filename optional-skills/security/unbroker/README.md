...
The design is **Vaelis-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
Then start a new Vaelis session and drive it (below). The skill works zero-config; a few optional
...
Drive it from a Vaelis session:
...
- **PII handling.** Dossiers live under the Vaelis home directory (`0600`, optionally
