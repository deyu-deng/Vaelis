...
the actual `setup.sh` that creates the project workspace, configures Vaelis
...
PyYAML must be installed in the user's Python (it ships with most Vaelis
...
the Vaelis `.env` (`${HERMES_HOME:-$HOME/.hermes}/.env`) and macOS Keychain
(if on macOS):
