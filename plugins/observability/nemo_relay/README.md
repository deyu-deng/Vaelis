...
Optional Vaelis observability plugin that maps Vaelis observer hooks to
...
not replace Vaelis Agent's planner, tools, memory, model provider routing, or
CLI UX. Instead, this plugin lets Vaelis emit NeMo Relay lifecycle events for
the work Vaelis already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, Vaelis Agent can:

- Preserve Vaelis execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
`HERMES_HOME` is the Vaelis profile/config home used by both
`hermes plugins enable ...` and the later `hermes chat ...` run. If unset,
Vaelis uses the user's default home, usually `~/.hermes`. For isolated smoke
...
`HERMES_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Vaelis does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
Vaelis keeps the direct env-var fallbacks active for that run.
...
exposes `llm.execute(...)` / `tools.execute(...)`, Vaelis routes LLM and tool
...
For the full generic Vaelis middleware contract, see
...
This run starts a parent Vaelis session, delegates to a child subagent, has the
...
export HERMES_NEMO_RELAY_ATIF_AGENT_NAME='Vaelis Agent E2E'
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Vaelis Agent E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Vaelis dispatches the read-only tools as one batch, and NeMo Relay
...
- Vaelis sessions map to `agent` scopes.
- Vaelis API request hooks map to `llm` scope start/end events.
- Vaelis tool hooks map to `tool` scope start/end events.
...
The `observability/nemo_relay` plugin uses Vaelis execution middleware to hand
...
When the adaptive component is enabled and the installed NeMo Relay runtime
exposes `llm.execute(...)` and `tools.execute(...)`, Vaelis routes execution
through these boundaries:

```text
Vaelis provider call
  -> llm_execution middleware
    -> nemo_relay.llm.execute(...)
      -> Vaelis provider adapter next_call(...)

Vaelis tool call
  -> tool_execution middleware
    -> nemo_relay.tools.execute(...)
      -> Vaelis tool dispatcher next_call(...)
```
...
middleware for a local Vaelis run. This path requires a NeMo Relay runtime that
...
agent_name = "Vaelis Middleware E2E"
agent_version = "local"
