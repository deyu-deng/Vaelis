"""langfuse — Vaelis plugin for Langfuse observability.

Traces Vaelis conversations, LLM calls, and tool usage to Langfuse.

Activation is handled by the Vaelis plugin system — standalone plugins only
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    Activation of this plugin is controlled by the Vaelis plugin system —
...
def _trace_key(
    ...
    *,
    ...
    Older Vaelis paths only expose ``task_id``/``session_id``. Newer paths
...
def on_pre_llm_call(*, task_id: str = "", session_id: str = "", platform: str = "", model: str = "",
    ...
    # Older Vaelis branches used pre_llm_call for request-scoped tracing and
    # passed the actual API messages. Current Vaelis also has a turn-scoped
    ...
    # messages is a list only for legacy Vaelis branches that fired
    # pre_llm_call with API messages directly. Current Vaelis fires
...
def register(ctx) -> None:
    ...
    # Vaelis versions.  pre_api_request / post_api_request fire per API
