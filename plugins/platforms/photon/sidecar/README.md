...
Small Node helper that bridges Vaelis Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Vaelis is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Vaelis-Sidecar-Token`)
...
The Vaelis plugin's `hermes photon setup` command runs `npm install`
here automatically.
