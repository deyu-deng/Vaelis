...
This plugin connects Vaelis Agent to iMessage (and other Spectrum
...
TypeScript-only, Vaelis runs it inside a small supervised Node sidecar and
...
- **Outbound**: `send` / `send_typing` / reaction tapbacks are loopback POSTs
  ...
  `/unreact`), authenticated with a shared `X-Vaelis-Sidecar-Token`.
...
2. **Find or create** the `Vaelis Agent` project on the Photon dashboard.
...
There is no separate `login` command; like every other Vaelis channel,
