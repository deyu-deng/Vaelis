# Vaelis Agent Security Policy

This document describes Vaelis Agent's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/deyu-deng/Vaelis/security/advisories/new)
or **[安全联系邮箱占位]**. Do not open public issues for
security vulnerabilities. **Vaelis Agent does not operate a bug
bounty program.**
...
Vaelis Agent is a single-tenant personal agent. Its posture is
layered, and the layers are not equally load-bearing. Reporters and
operators should reason about them in the same terms.

### 2.1 Definitions

- **Agent process.** The Python interpreter running Vaelis Agent,
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Vaelis Agent access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Vaelis Agent's documentation
...
Vaelis Agent supports two OS-level isolation postures. They address
...
Vaelis Agent supports this in two ways:

- **Vaelis Agent's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Vaelis Agent's in-process heuristics
...
Vaelis Agent filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Vaelis Agent
itself. Bugs in Vaelis Agent's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Vaelis Agent does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Vaelis Agent's own documentation, or reasonable
  operator expectations would predict — including cases where
  Vaelis Agent has documented a stance about how its output should
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Vaelis Agent's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Vaelis Agent. Bugs in Vaelis
...
- Vaelis Agent includes supply-chain guards for MCP server
