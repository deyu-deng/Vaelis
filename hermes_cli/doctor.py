...
Diagnoses issues with Vaelis Agent setup.
"""
...
def run_doctor(args):
    ...
    print(color("│                 🩺 Vaelis Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    try:
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses Vaelis' own device-code flow — the Codex CLI is
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Vaelis a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Vaelis Agent Persona\n\n"
                "<!-- Edit this file to customize how Vaelis communicates. -->\n\n"
                "You are Vaelis, a helpful AI assistant.\n",
                encoding="utf-8",
            )
            ...
            try:
                ...
                if total == 0:
                    ...
                    if audit_extra and audit_extra[0] == "--workspace":
                        ...
                        # tree — in that case it is an npm bug, not a Vaelis one.
                        check_info(
