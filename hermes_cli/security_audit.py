"""On-demand supply-chain audit for Vaelis Agent installs.

Scans three surfaces a Vaelis user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Vaelis venv (every PyPI dist via ``importlib.metadata``).
