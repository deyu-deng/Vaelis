"""
Vaelis Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Vaelis.app`` or ``~/Applications/Vaelis.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Vaelis`` (NSIS per-user)
...
the app name ("Vaelis"), separate from ``$HERMES_HOME``:
  - macOS:   ``~/Library/Application Support/Vaelis``
  - Windows: ``%APPDATA%\\Vaelis``
  - Linux:   ``$XDG_CONFIG_HOME/Vaelis`` (default ``~/.config/Vaelis``)
...
def desktop_userdata_dir() -> Path:
    ...
    Mirrors Electron's ``app.getPath('userData')`` for an app named "Vaelis"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Vaelis"
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Vaelis" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Vaelis.app"),
            home / "Applications" / "Vaelis.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Vaelis).
            local_base / "Programs" / "Vaelis",
        ...
        if program_files:
            ...
            paths.append(Path(program_files) / "Vaelis")
    else:
        ...
        paths += [
            ...
            data_base / "applications" / "Vaelis.desktop",
        ]
    return paths
