...
def _catalog_root() -> Path:
    """Return the optional-mcps/ directory shipped with this Vaelis install."""
...
def _parse_manifest(path: Path) -> CatalogEntry:
    ...
    if mv != _MANIFEST_VERSION:
        raise CatalogError(
            ...
            f"(this Vaelis understands version {_MANIFEST_VERSION})"
        )
...
def list_catalog() -> List[CatalogEntry]:
    ...
    UIs can tell the user their Vaelis is out of date.
    """
    root = _catalog_root()
    if not root.exists():
        return []
...
def catalog_diagnostics() -> List[tuple]:
    ...
    is one of:
      - ``future_manifest`` — manifest_version is newer than this Vaelis
        understands. Update Vaelis to install this entry.
...
def install_entry(entry: CatalogEntry, *, enable: bool = True) -> None:
    ...
    print(color(
        ...
        f"Start a new Vaelis session to load its tools.",
        Colors.GREEN,
    ))
