...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Vaelis profiles via PID files."""
    _exclude = set(exclude_pids or set())
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our Vaelis Docker image — that one runs s6-overlay as
...
SERVICE_DESCRIPTION = "Vaelis Agent Gateway - Messaging Platform Integration"
...
def _profile_arg(hermes_home: str | None = None, default_root: str | Path | None = None) -> str:
    ...
    Args:
        ...
        default_root: Optional Vaelis root to compare against. Used when
...
# Legacy service names from older Vaelis installs that predate the
...
def _find_legacy_hermes_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Vaelis gateway units.

    Detects unit files installed by older Vaelis versions that used a
...
def has_legacy_hermes_units() -> bool:
    """Return True when any legacy Vaelis gateway unit files exist."""
    return bool(_find_legacy_hermes_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Vaelis gateway unit files if any are installed.
    ...
    if not legacy:
        return
    print_warning("Legacy Vaelis gateway unit(s) detected from an older install:")
...
def remove_legacy_hermes_units(
    ...
    """Stop, disable, and remove legacy Vaelis gateway unit files.
    ...
    legacy = _find_legacy_hermes_units()
    if not legacy:
        print("No legacy Vaelis gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy Vaelis gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Vaelis often sets ``HOME`` to a profile-scoped directory, but
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(
        f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Vaelis install"
    )
    return True
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print(
        "↻ Updated gateway launchd service definition to match the current Vaelis install"
    )
    return True
...
def launchd_status(deep: bool = False):
    ...
    # Vaelis PID tracking — may be a detached fallback process spawned when
    ...
    # launchd PID and the Vaelis PID file).
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Vaelis install")
    else:
        print("⚠ Service definition is stale relative to the current Vaelis install")
...
def _guard_official_docker_root_gateway() -> None:
    ...
    print_error(
        "Refusing to run the Vaelis gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'hermes' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/hermes/docker/entrypoint.sh before the Vaelis command."
    )
    print(
...
_PLATFORMS = [
    ...
    # plugins/platforms/matrix/adapter.py::register(). #41112.
    {
        ...
        "vars": [
            ...
            },
            {
                ...
                "password": False,
                "help": "Channel ID where Vaelis delivers cron results and notifications.",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Vaelis connects via the BlueBubbles REST API and receives",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Vaelis will connect automatically over WebSocket",
        ],
        "vars": [
            {
                "name": "YUANBAO_APP_ID",
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Vaelis will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info(
        "  3. Vaelis will store the returned account_id/token in ~/.hermes/.env."
    )
    print_info(
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        # Stop, disable, and remove legacy Vaelis gateway unit files from
