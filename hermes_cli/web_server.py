...
def _app_token_path(hermes_home: Path) -> Path:
    """Absolute path to the persistent App token file under *hermes_home*.
    ...
    if parent.exists() and not parent.is_dir():
        ...
        return Path(hermes_home) / APP_TOKEN_REL_DIR / APP_TOKEN_FILENAME
    return target
...
def _load_or_mint_app_token(hermes_home: Path) -> str | None:
    ...
    """
    token_path = _app_token_path(hermes_home)
...
def _verify_token(presented: str, expected: str) -> bool:
    ...
    that don't want to construct a fake ``Request``.
    """
...
try:
    ...
    _APP_TOKEN: str | None = _load_or_mint_app_token(_get_hermes_home())
...
#
# Applied at the outermost middleware (registered last, runs first) so it
# tags responses regardless of which gate (cookie / token / SPA) handled
# the request — and so it covers /api/status (which auth lets through
# without a token) and the gated routes alike. SPA HTML and static asset
# responses also get the header (single line of code; useful when a
# tablet pinned the URL in a WebView and pings it during boot).
# ---------------------------------------------------------------------------
@app.middleware("http")
...
def _fs_default_cwd(profile_home: Optional[Path] = None) -> str:
    """Resolve the file API's default cwd.

    Order (WP-FS-PROFILE-CWD):

    1. ``profile_home`` — when given, defer to ``_profile_configured_cwd``
       so a non-launch profile's ``terminal.cwd`` wins over the stale
       launch-profile env var (issue #40334 — desktop backend serves every
       profile, but ``TERMINAL_CWD`` only reflects the launch profile).
    2. ``load_config().terminal.cwd`` / ``TERMINAL_CWD`` env — the launch
       profile's workspace.
    3. ``Path.cwd()`` — wherever the backend was started.

    Placeholders (``.``/``auto``/``cwd``) and non-existent directories never
    pretend to be a workspace — they fall through silently rather than
    returning a string that would mislead the file API caller.
    """
    if profile_home is not None:
        try:
            from tui_gateway.server import _profile_configured_cwd
...
async def fs_default_cwd(profile: str | None = None):
    """Return the workspace cwd the file API should default to.

    When the caller passes ``?profile=<name>`` we resolve that profile's
    ``terminal.cwd`` first; otherwise the launch-profile value (env /
    ``load_config().terminal.cwd``) is used. Both layers ignore the
    ``.`` / ``auto`` / ``cwd`` placeholders and any non-existent path so
    the dashboard never shows a workspace that cannot be opened.
    """
    ...
    if profile:
        ...
        try:
            ...
            canon = None
        if canon and profiles_mod.profile_exists(canon):
            profile_home = profiles_mod.get_profile_dir(canon)
    cwd = _fs_default_cwd(profile_home)
...
# create_job path. See tools/blueprints.py for the single source of truth.
# ---------------------------------------------------------------------------
...
async def list_cron_blueprints():
    ...
    """
    try:
        from tools.blueprints import CATALOG, blueprint_catalog_entry
...
async def instantiate_blueprint(body: AutomationBlueprintInstantiate, profile: str = "default"):
    ...
    try:
        from tools.blueprints import fill_blueprint, get_blueprint, BlueprintFillError
...
def start_server(
    ...
    # read-only ``$HERMES_HOME`` could expose the dashboard unauthenticated
