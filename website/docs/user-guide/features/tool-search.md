...
**Tool Search** is Vaelis' opt-in progressive-disclosure layer for that
...
:::info Built-in Vaelis tools never defer
The tools that make up Vaelis' core capability set (`terminal`,
...
When the model invokes `tool_call`, Vaelis **unwraps the bridge** and
...
- **No JS sandbox.** Vaelis uses the simpler "structured tools" mode
