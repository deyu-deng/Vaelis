...
Vaelis runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Vaelis
...
1. Vaelis captures a baseline of current diagnostics for the file.
...
…). Vaelis auto-detects the binary on PATH or in
`<HERMES_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point Vaelis at the extracted bundle — the directory that contains
...
`node_modules` tree — Vaelis installs both packages together when you
...
When `install_strategy: auto`, Vaelis installs binaries into
...
shared location — the staging dir is fully Vaelis-owned and is
...
Servers are kept alive for the life of the Vaelis process. There's
