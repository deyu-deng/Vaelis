...
description: "Customize Vaelis Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Vaelis Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.
...
If you want to change who Vaelis is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Vaelis now seeds a default `SOUL.md` automatically in:

```text
~/.hermes/SOUL.md
```

More precisely, it uses the current instance's `HERMES_HOME`, so if you run Vaelis with a custom home directory, it will use:
...
- Vaelis creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Vaelis loads `SOUL.md` only from `HERMES_HOME`
- Vaelis does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Vaelis falls back to a built-in default identity
...
If Vaelis loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `HERMES_HOME`, the personality belongs to the Vaelis instance itself.

That also makes it easier to teach users:
- "Edit `~/.hermes/SOUL.md` to change Vaelis' default personality."
...
- how Vaelis should handle uncertainty, disagreement, or ambiguity
...
## What Vaelis injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Vaelis falls back to a built-in default identity ("You are Vaelis Agent, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Vaelis ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Vaelis, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Vaelis its persistent default personality unless the overlay meaningfully changes it.
...
- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Vaelis speaks
- `display.skin` and `/skin` affect how Vaelis looks in the terminal
