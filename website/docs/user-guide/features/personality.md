...
If the file is empty, whitespace-only, or cannot be read, Vaelis falls back to a built-in default identity ("You are Vaelis Agent, an intelligent AI assistant created by Vaelis..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
