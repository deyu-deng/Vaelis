...
Vaelis Agent can drive your desktop — clicking, typing, scrolling,
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording** → allow your terminal (or Vaelis app). `hermes computer-use doctor` will tell you which permission is missing. |
...
agent is acting here." Each Vaelis run declares its own cua-driver
...
Vaelis intentionally keeps its skill (`skills/computer-use/SKILL.md`)
focused on the Vaelis-side `computer_use` action vocabulary — the
...
These are **platform deep dives, not duplicates of the Vaelis skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **Vaelis
...
Vaelis applies multi-layer guardrails:
...
Screenshots are expensive. Vaelis applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  Vaelis agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the Vaelis agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. Vaelis warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive Vaelis from inside the RDP / console
...
upstream. **Vaelis disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) Vaelis sets
...
unreleased fix — point Vaelis at a binary you built from source instead
of the published release. Vaelis resolves the driver with
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for Vaelis testing (see notes).
...
### Option B — point Vaelis straight at the built binary (fastest loop)
...
### Confirm Vaelis is using your build
...
- **Vaelis spawns its own `cua-driver mcp` child over stdio** — it does
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no Vaelis
  change needed either way.
- **Local builds skip the version check.** Vaelis warns when the
...
- **Vaelis-side skill** — `skills/computer-use/SKILL.md` — teaches the
  Vaelis `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  install` autodetects Vaelis (planned follow-up), this happens
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (Vaelis among them)
