...
files. It is **never persisted** to any `.env` by Vaelis.
...
If you try to change a managed value, Vaelis refuses and names the source:
...
Changes take effect on the next Vaelis start (a malformed managed file is logged
...
- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Vaelis as `root`), managed scope is advisory.
