...
sidebar_position: 3
title: "Desktop App"
description: "The native Vaelis desktop app — a polished experience for chatting with Vaelis, with streaming tool output, side-by-side previews, a file browser, voice, cron, profiles, skills, and settings. macOS, Windows, and Linux."
---

# Desktop App

The Vaelis desktop app is a native app built around the **same** agent you get from the CLI and the gateway — same config, same API keys, same sessions, same skills, same memory. It is not a separate product or a lightweight clone; it uses the same Vaelis Agent core and settings, and drives it through a modern & thoughtfully designed UI. If you have used `hermes` in a terminal, everything you set up there is already here, and anything you do here shows up there.

It runs on **macOS, Windows, and Linux**.

:::tip Which interface is which?
Vaelis has several front ends that all talk to the same agent:
...
Follow the [installation instructions for Vaelis Desktop](../getting-started/installation.md).

If you already have Vaelis installed, simply run
...
- **The same conversation history** as every other Vaelis surface — sessions started here resume in the CLI/TUI and vice versa.
...
Talk to Vaelis and hear it back, the same [voice mode](./features/voice-mode.md) available elsewhere. On macOS the OS will prompt once for microphone access.
...
The app also surfaces the broader Vaelis management surface so you don't have to drop to a terminal:
...
- **Profiles** — switch between [Vaelis profiles](./profiles.md) (isolated config/skills/sessions).
...
The [manual update process](https://vaelis.dev/docs/getting-started/updating) also works with the GUI.

## Uninstalling

Open **Settings → About → Danger zone** and pick how much to remove:

- **Uninstall Chat GUI only** — removes the desktop app and its data; the Vaelis agent, your config, and your chats stay. (Same as `hermes uninstall --gui`.)
...
| `--hermes-root PATH` | Override the Vaelis source root the app uses (sets `HERMES_DESKTOP_HERMES_ROOT`)          |
...
The rest of this section shows the username/password path because it's the quickest to stand up on a trusted network; for the OAuth path see [Web Dashboard → Default provider: Vaelis](./features/web-dashboard.md#default-provider-nous-research).
...
For the same setup from the web-dashboard angle, see [Web Dashboard → Connecting Vaelis Desktop to a remote backend](./features/web-dashboard.md#connecting-hermes-desktop-to-a-remote-backend); the env vars are catalogued under [Environment Variables → Web Dashboard & Vaelis Desktop](../reference/environment-variables.md#web-dashboard--hermes-desktop).
...
tccutil reset Microphone com.deyu-deng.vaelis
```
