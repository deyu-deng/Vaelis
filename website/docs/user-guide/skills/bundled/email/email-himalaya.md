...
This skill is separate from the Vaelis Email gateway adapter. The gateway
adapter lets people email the agent and uses Vaelis' built-in IMAP/SMTP
...
To reply non-interactively from Vaelis, read the original message, compose a reply, and pipe it:
...
**Non-interactive (use this from Vaelis)** — pipe the message via stdin:
