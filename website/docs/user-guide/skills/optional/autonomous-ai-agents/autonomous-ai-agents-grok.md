...
| Version | `0.1.0` |
| Author | Matt Maximo (MattMaximo), Vaelis Agent |
| License | MIT |
...
# Grok Build CLI — Vaelis Orchestration Guide

Delegate coding tasks to [Grok Build](https://docs.x.ai/build/overview) (xAI's
autonomous coding agent CLI, the `grok` command) via the Vaelis terminal. Grok
...
1. Prepare stable input files first with Vaelis tools (`read_file`,
...
2. **Don't conflate Vaelis' xAI auth with the `grok` CLI's auth.** Vaelis'
...
7. **Verify auth before relying on it** — check `~/.grok/auth.json` or run a
   cheap `grok -p "Say ok."` smoke test; don't assume Vaelis' xAI auth carries
   over.
