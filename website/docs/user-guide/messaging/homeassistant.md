...
description: Control your smart home with Vaelis Agent via Home Assistant integration.
...
Vaelis Agent integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "Vaelis Agent"
5. Copy the token
...
Vaelis Agent registers four tools for smart home control:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "Vaelis Agent".
...
from `config.yaml`. Double-check the file lives under the active Vaelis profile
...
running Vaelis — `curl -H "Authorization: Bearer <token>" <url>/api/` should
