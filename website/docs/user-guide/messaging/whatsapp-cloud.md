...
description: "Set up Vaelis Agent as a WhatsApp bot via Meta's official Business Cloud API"
---

# WhatsApp Business Cloud API Setup

Vaelis can connect to WhatsApp through Meta's **official** WhatsApp Business Cloud API. This is the production-grade path: no Node.js bridge subprocess, no QR codes, no account-ban risk.
...
- The Vaelis gateway needs a **public HTTPS URL** so Meta can deliver inbound messages via webhook.
- Replies more than 24 hours after the user's last message require a pre-approved **template** (this is Meta's "customer service window" rule, not a Vaelis limit).
...
4. **Optional but recommended**: ffmpeg on `PATH` so outbound voice messages render as native WhatsApp voice-note bubbles (green waveform) instead of MP3 audio attachments. Vaelis degrades gracefully if absent.
...
## Exposing Vaelis to the internet

The Cloud API delivers inbound messages by HTTPS POST to your webhook URL — that means the Vaelis gateway has to be reachable from Meta's servers.  Three common ways:
...
3. Start the Vaelis gateway: `hermes gateway`.
...
## Allowlist (Vaelis-side)

In addition to Meta's recipient whitelist, Vaelis has its own per-platform allowlist that controls **which incoming messages the agent processes**.  Add to `~/.hermes/.env`:
...
When the agent invokes any of these flows, Vaelis uses WhatsApp's native interactive messages — tap-to-answer buttons instead of "reply with the number" prompts:
...
Vaelis acknowledges inbound messages immediately:
...
Vaelis TTS produces MP3. Two paths:
...
Vaelis warns the agent about this window in its system prompt, so the model knows to mention it when scheduling delayed messages.

Message-template support (the workaround for outside-window sends) is not yet implemented in Vaelis. If you need it, please [open an issue](https://github.com/NousResearch/hermes-agent/issues) — it's planned but waiting on a clear demand signal.

### Group chats

The Cloud API has limited group support (capability-tier gated by Meta).  Vaelis's `whatsapp_cloud` adapter currently handles **direct messages only** in v1.  If you need group chats, use the Baileys bridge.

### Outbound rate limit

Meta's default throughput is **80 messages/second per business phone number**, with upgrades available.  Vaelis doesn't currently enforce this client-side — extremely high-volume sends could hit Meta's limit.
...
- **App Secret not set** — without it, Vaelis refuses inbound POSTs with 503.  Meta interprets that as "can't validate."
...
- Wait for template support to land in Vaelis.
...
- **Treat the App Secret like a password** — anyone with it can forge webhook payloads that Vaelis will accept as authentic.
...
Most users running Vaelis for personal projects prefer Baileys. Most users running customer-facing bots prefer Cloud API.
