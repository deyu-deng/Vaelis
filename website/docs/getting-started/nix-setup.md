...
description: "Install and deploy Vaelis Agent with Nix — from quick `nix run` to fully declarative NixOS module with container mode"
---
...
Vaelis Agent ships a Nix flake & a NixOS module.
...
```bash
# Run the desktop app
nix run github:Vaelis/hermes-agent#desktop

# Or install persistently
nix profile install github:Vaelis/hermes-agent#desktop

# run the tui
nix run github:Vaelis/hermes-agent -- setup
nix run github:Vaelis/hermes-agent -- --tui

# or install it in your profile
nix profile install github:Vaelis/hermes-agent
hermes setup
hermes --tui
```
...
# /etc/nixos/flake.nix (or your system flake)
{
  inputs = {
    ...
    hermes-agent.url = "github:Vaelis/hermes-agent";
  };
...
:::note Model naming
`settings.model.default` uses the model identifier your provider expects. With [OpenRouter](https://openrouter.ai) (the default), these look like `"anthropic/claude-sonnet-4"` or `"google/gemini-3-flash"`. If you're using a provider directly (Anthropic, OpenAI), set `settings.model.base_url` to point at their API and use their native model IDs (e.g., `"claude-sonnet-4-20250514"`). When no `base_url` is set, Vaelis defaults to OpenRouter.
:::
...
Both `environment` (non-secret vars) and `environmentFiles` (secret files) are merged into `$HERMES_HOME/.env` at activation time (`nixos-rebuild switch`). Vaelis reads this file on every startup, so changes take effect with a `systemctl restart hermes-agent` — no container recreation needed.
...
The `documents` option installs files into the agent's working directory (the `workingDirectory`, which the agent reads as its workspace). Vaelis looks for specific filenames by convention:

- **`USER.md`** — context about the user the agent is interacting with.
- Any other files you place here are visible to the agent as workspace files.

The agent identity file is separate: Vaelis loads its primary `SOUL.md` from `$HERMES_HOME/SOUL.md`, which in the NixOS module is `${services.hermes-agent.stateDir}/.hermes/SOUL.md`. Putting `SOUL.md` in `documents` only creates a workspace file and will not replace the main persona file.
...
Set `auth = "oauth"` for servers using OAuth 2.1. Vaelis implements the full PKCE flow — metadata discovery, dynamic client registration, token exchange, and automatic refresh.
...
The first OAuth authorization requires a browser-based consent flow. In a headless deployment, Vaelis prints the authorization URL to stdout/logs instead of opening a browser.
...
Plugins are symlinked into `$HERMES_HOME/plugins/` at activation time. Vaelis discovers them via its normal directory scan. Removing a plugin from the list and running `nixos-rebuild switch` removes the symlink.
...
```nix
{
  inputs.hermes-agent.url = "github:Vaelis/hermes-agent";
...
|---|---|---|---|
| `/nix/store` | `/nix/store` | `ro` | Vaelis binary + all Nix deps |
