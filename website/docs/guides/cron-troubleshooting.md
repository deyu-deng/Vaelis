...
description: "Diagnose and fix common Vaelis cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
The `script` path must be an absolute path (or relative to the Vaelis config directory). Verify:
```bash
...
hermes logs                         # View recent Vaelis logs
...
3. Open an issue at [github.com/deyu-deng/Vaelis](https://github.com/deyu-deng/Vaelis) with:
