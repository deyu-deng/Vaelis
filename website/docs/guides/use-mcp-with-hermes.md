---
sidebar_position: 6
title: "Use MCP with Vaelis"
description: "A practical guide to connecting MCP servers to Vaelis Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Vaelis

This guide shows how to actually use MCP with Vaelis Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Vaelis tool
- you want Vaelis to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Vaelis to internal APIs, databases, or company systems without modifying Vaelis core

Do not use MCP when:
- a built-in Vaelis tool already solves the job well
...
- Vaelis remains the agent
- MCP servers contribute tools
- Vaelis discovers those tools at startup or reload time
...
If you installed Vaelis with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Vaelis banner/status should show MCP integration when configured
- ask Vaelis what tools it has available
...
## WSL2: bridge Vaelis in WSL to Windows Chrome

This is the practical setup when:

- Vaelis runs inside WSL2
...
In this setup, Vaelis does **not** connect to Chrome directly. Instead:

- Vaelis runs in WSL
- Vaelis starts a local stdio MCP server
...
```text
Vaelis (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Vaelis stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Vaelis core browser transport
...
Then start a fresh Vaelis session or run:
...
Once loaded, Vaelis can use the MCP-prefixed browser tools directly. For example:
...
If Vaelis runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Vaelis from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Vaelis from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Vaelis:
...
2. Vaelis-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Vaelis will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Vaelis to reason over a bounded workspace.
...
Use [Open Scaffold](https://github.com/graphanov/open-scaffold) when you want Vaelis to read a repository's durable AI-work record: mission, plans, evidence notes, handoff packets, and review/gate results. Vaelis remains the agent; Open Scaffold remains the repo-local record.
...
- Its write tools require the server to be started with `--allow-write`; do not enable that until you explicitly want Vaelis to mutate `.osc` files.
- Open Scaffold records and gates work; it does not authorize Vaelis to merge, publish, deploy, or spawn runtimes.
...
Now Vaelis can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Vaelis core.
...
Because Vaelis now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
