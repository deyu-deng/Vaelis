---
sidebar_position: 8
title: "Use Voice Mode with Vaelis"
description: "A practical guide to setting up and using Vaelis voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Vaelis
...
- you want Vaelis sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Vaelis.
...
## Step 1: make sure normal Vaelis works first
...
Vaelis supports both local and cloud speech stacks.
...
If you choose NeuTTS in the setup wizard, Vaelis checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Vaelis transcribes and responds
...
- asking Vaelis to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Vaelis loop.

## Tuning CLI behavior

### Silence threshold

If Vaelis starts/stops too aggressively, tune:
...
Vaelis stays a normal chat bot, but can speak replies.
...
- you want Vaelis to function like a portable research or ops assistant
...
Vaelis joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Vaelis detects speech boundaries
- transcripts are posted in the associated text channel
- Vaelis responds in text and audio
