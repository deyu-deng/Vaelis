...
messaging platform Vaelis is already configured for. Think of it as a
...
Any platform Vaelis ships adapters for works as a target:
...
When stdin is a TTY (no pipe), Vaelis does **not** wait for input — you'll
...
Because `hermes send` reuses your Vaelis config, the same script works on
any host where Vaelis is installed — no need to export bot tokens into
...
| Approach | Multi-platform | Reuses Vaelis creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
