#!/usr/bin/env python3
"""Generate llms.txt and llms-full.txt for the Vaelis docs site.
...
Both publish at:
  https://vaelis.dev/docs/llms.txt
  https://vaelis.dev/docs/llms-full.txt
...
SITE_BASE = "https://vaelis.dev/docs"
...
SECTIONS: list[tuple[str, list[tuple[str, str, str | None]]]] = [
    ...
    ("Guides & Tutorials", [
        ...
        ("guides/python-library", "Use Vaelis as a Python Library", None),
        ("guides/use-mcp-with-hermes", "Use MCP with Vaelis", None),
        ("guides/use-voice-mode-with-hermes", "Use Voice Mode with Vaelis", None),
        ("guides/use-soul-with-hermes", "Use SOUL.md with Vaelis", None),
        ("guides/build-a-hermes-plugin", "Build a Vaelis Plugin", None),
    ...
    ("Reference", [
        ...
        ("reference/skills-catalog", "Bundled Skills Catalog", "Table of all ~90 skills bundled with Vaelis"),
...
def emit_llms_index() -> str:
    ...
    lines: list[str] = []
    lines.append("# Vaelis Agent")
    lines.append("")
    lines.append(
        "> The self-improving AI agent built by Vaelis. A terminal-native "
    ...
    lines.append("")
    lines.append(
        "Install: `curl -fsSL https://raw.githubusercontent.com/Vaelis/"
        "hermes-agent/main/scripts/install.sh | bash`  "
        "(Linux, macOS, WSL2, Termux)"
    )
    lines.append("")
    lines.append("Repo: https://github.com/deyu-deng/Vaelis")
    lines.append("")
...
def emit_llms_full() -> str:
    ...
    seen: set[Path] = set()
    chunks: list[str] = [
        "# Vaelis Agent — Full Documentation\n",
        (
            "This file is the entire Vaelis Agent documentation concatenated for LLM "
            "context ingestion. Section order reflects docs-site navigation: Getting "
            "Started, Using Vaelis, Features, Messaging, Integrations, Guides, "
            "Developer Guide, Reference, then everything else.\n"
        ),
        "Canonical site: https://vaelis.dev/docs\n",
        "Short index: https://vaelis.dev/docs/llms.txt\n",
        "\n---\n\n",
    ]
