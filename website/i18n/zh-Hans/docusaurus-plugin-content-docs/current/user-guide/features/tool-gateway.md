...
:::tip 快速开始
Tool Gateway 包含在付费 Nous Portal 订阅中。**[管理订阅 →](https://vaelis.dev/manage-subscription)**
:::

**Tool Gateway** 让已付费的 [Nous Portal](https://vaelis.dev) 用户通过同一份订阅，直接使用网页搜索、文生图、语音合成（TTS）与浏览器自动化，而**不必**再分别注册 Firecrawl、FAL、OpenAI、Browser Use 等服务的 API Key。
...
Tool Gateway 仅对 **[付费](https://vaelis.dev/manage-subscription)** Nous Portal 订阅开放；免费档不可用——请 [升级订阅](https://vaelis.dev/manage-subscription) 后解锁。
...
运行 `hermes model` 并选择 Nous Portal 作为提供商时，Vaelis 会主动询问是否启用 Tool Gateway：
...
```bash
TOOL_GATEWAY_DOMAIN=vaelis.dev     # 网关路由基础域名
...
经网关路由的工具会停止工作，直到你 [续订](https://vaelis.dev/manage-subscription) 或通过 `hermes tools` 改回直连 Key。
