...
function assistantBriefingMessage(running = true): ThreadMessage {
  return {
    id: `assistant-briefing-${running ? 'running' : 'done'}`,
    role: 'assistant',
    content: [
      {
        type: 'tool-call',
        ...
        ...(running
          ...
          : {
              result: {
                ...
                intent: 'write_briefing',
                agent: { id: 'agenda' },
                briefing: 'Tomorrow: 09:00 standup.',
                route: 'fallback',
                model: 'test'
              }
            })
...
function assistantDispatchMessage(running = true): ThreadMessage {
  return {
    id: `assistant-dispatch-${running ? 'running' : 'done'}`,
    role: 'assistant',
    content: [
      {
        type: 'tool-call',
...
describe('assistant-ui streaming renderer', () => {
  ...
  it('keeps a running secretary_ask tool row visible while the thread is busy', async () => {
    const { container } = render(<RunningMessageHarness message={assistantDispatchMessage(true)} />)

    await waitFor(() => {
      expect(container.textContent).toContain('Asking 日程秘书')
    })
    expect(container.querySelector('[data-slot="tool-block"]')).toBeTruthy()
  })

  it('shows write_briefing route=workbuddy on the waiting tool row', async () => {
    const { container } = render(<RunningMessageHarness message={assistantBriefingMessage(true)} />)

    await waitFor(() => {
      expect(container.textContent).toContain('Asking 日程秘书')
      expect(container.textContent).toContain('route=workbuddy')
    })
  })

  it('shows write_briefing route=fallback on the finished tool row', async () => {
    const { container } = render(<MessageHarness message={assistantBriefingMessage(false)} />)

    await waitFor(() => {
      expect(container.textContent).toContain('Asked 日程秘书')
      expect(container.textContent).toContain('route=fallback')
    })
  })
})
