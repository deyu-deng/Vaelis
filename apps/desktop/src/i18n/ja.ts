...
export const ja = defineLocale({
  ...
  boot: {
    ready: 'Vaelis Desktop の準備ができました',
    desktopBootFailedWithMessage: message => `デスクトップの起動に失敗しました: ${message}`,
    steps: {
      connectingGateway: 'ライブデスクトップゲートウェイに接続中',
      loadingSettings: 'Vaelis の設定を読み込み中',
      loadingSessions: '最近のセッションを読み込み中',
      startingDesktopConnection: 'デスクトップ接続を開始中',
      startingHermesDesktop: 'Vaelis Desktop を起動中…'
    },
    errors: {
      backgroundExited: 'Vaelis バックグラウンドプロセスが終了しました。',
      backgroundExitedDuringStartup: '起動中に Vaelis バックグラウンドプロセスが終了しました。',
  ...
  notifications: {
    ...
    backendOutOfDateMessage:
      'Vaelis バックエンドがこのデスクトップビルドより古く、正常に動作しない場合があります。更新して揃えてください。',
    installMethodUnsupportedTitle: 'サポート対象外のインストール方法',
    updateHermes: 'Vaelis を更新',
    ...
    errors: {
      ...
      methodNotAllowed:
        'デスクトップバックエンドがそのリクエストを拒否しました (405 Method Not Allowed)。Vaelis Desktop を再起動してください。',
    ...
    native: {
      ...
      rejectAction: '拒否',
      inputTitle: '入力が必要です',
      inputBody: 'Vaelis が応答を待っています。',
      turnDoneTitle: 'Vaelis が完了しました',
  ...
  settings: {
    ...
    resetConfirm: 'すべての設定を Vaelis のデフォルトに戻しますか？',
    ...
    nav: {
      ...
      providerDesktopQuotas: 'デスクトップ割り当て',
      gateway: 'ゲートウェイ',
      apiKeys: 'ツールとキー',
      keysTools: 'ツール',
      keysSettings: '設定',
    ...
    notifications: {
      ...
      focusedHint: '完了通知は Vaelis がバックグラウンドにあるときのみ表示されます。',
      kinds: {
        approval: {
          label: '承認が必要',
        ...
        input: {
          label: '入力が必要',
          description: 'Vaelis が質問したか、パスワードやシークレットを必要としています。'
        },
        turnDone: {
          label: '応答完了',
          description: 'Vaelis がバックグラウンドのときにターンが完了しました。'
        },
        turnError: {
          label: 'ターン失敗',
    ...
    appearance: {
      ...
      colorMode: 'カラーモード',
      colorModeDesc: '固定モードを選ぶか、Vaelis をシステム設定に合わせます。',
      ...
      pet: {
        title: 'ペット',
        intro:
          'アプリ上に浮かぶ petdex のアニメーションマスコットを採用しましょう。ツール実行中は走り、成功すると喜び、エラーでしょんぼりと、Vaelis の状態に反応します。',
        restartHint:
          'ペット機能には再起動が必要です。この機能が追加される前に起動したアプリが動作中です。Vaelis を終了して再度開き、このページに戻ってください。',
        scaleTitle: 'サイズ',
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      timezone:
        'Vaelis がローカル時刻のコンテキストを必要とするときに使用します。空欄ならシステムのタイムゾーンを使います。',
      agent: {
        imageInputMode: '画像添付をモデルへ送る方法を制御します。',
        maxTurns: 'Vaelis が 1 回の実行を停止するまでのツール呼び出しターン上限です。'
      },
      terminal: {
      ...
      fileReadMaxChars: 'Vaelis が 1 回のファイル読み取りで取得できる最大文字数です。',
      approvals: {
        mode: '明示的な承認が必要なコマンドを Vaelis がどう扱うかを設定します。',
      ...
      updates: {
        nonInteractiveLocalChanges:
          'アプリから Vaelis 自身を更新するとき、ローカルのソース変更を保持するか破棄するかを選びます。ターミナル更新では常に確認されます。'
      }
    }),
    about: {
      ...
      automaticUpdatesDesc: 'Vaelis はバックグラウンドで自動的に更新を確認し、利用可能になったら通知します。',
    ...
    gateway: {
      ...
      intro:
        'Vaelis Desktop はデフォルトで独自のローカルゲートウェイを起動します。別のマシンや信頼できるプロキシの背後で既に動作している Vaelis バックエンドをこのアプリで制御する場合は、リモートゲートウェイを使用してください。以下でプロファイルを選択して、それぞれのリモートホストを設定します。',
      appliesTo: '適用対象',
      ...
      localDesc:
        'ローカルホストでプライベートな Vaelis バックエンドを起動します。これがデフォルトで、オフラインでも動作します。',
      remoteTitle: 'リモートゲートウェイ',
      remoteDesc:
        'このデスクトップシェルをリモートの Vaelis バックエンドに接続します。ホスト型ゲートウェイは OAuth またはユーザー名とパスワードを使用します。自己ホスト型はセッショントークンを使用する場合があります。',
      ...
      restartingMessage: 'Vaelis Desktop は保存された設定を使用して再接続します。',
      savedMessage: '次回起動時に保存されます。',
      connectedTo: (baseUrl, version) => `${baseUrl}${version ? ` · Vaelis ${version}` : ''} に接続しました`,
    ...
    providers: {
      ...
      intro:
        'サブスクリプションでサインインします。API キーのコピーは不要です。Vaelis がアプリ内でブラウザーサインインを代行します。',
      connected: '接続済み',
      collapse: '折りたたむ',
      ...
      desktopQuotas: {
        title: 'デスクトップ割り当て',
        intro: 'Vaelis がルーティングできるサブスクリプションを持つデスクトップアプリを接続します。一度サインインすれば、残りは Vaelis がバックグラウンドで処理します。',
        connect: '接続',
        connected: '接続済み',
        notConnected: '未接続',
        connectError: provider => `${provider} に接続できませんでした`,
        notVerified: 'このアプリの接続取得はまだ検証されていません',
        useHub: 'このハブを使用',
        hubRunning: 'ハブ稼働中',
        hubStopped: 'ハブ停止中'
      }
  ...
  starmap: {
    ...
    emptyDesc: 'Vaelis がスキルやメモリを蓄積すると、ここに表示されます。'
  },
  agents: {
    close: 'エージェントを閉じる',
    title: 'スポーンツリー',
  ...
  commandCenter: {
    ...
    pets: {
      ...
      staleBackend: 'ペット機能を使うには Vaelis を再起動してください。',
      empty: '一致するペットがありません。',
      turnOff: 'オフ',
      turnOn: 'オン',
      installed: 'インストール済み',
    ...
    generatePet: {
      ...
      staleBackend: 'ペットを生成するには Vaelis を更新してください。',
      backgroundHint: 'このウィンドウは閉じても大丈夫です。完了したら Vaelis が通知します。',
    ...
    nav: {
      ...
      settings: { title: '設定', detail: 'Vaelis デスクトップを設定' },
    ...
    hermesActiveSessions: (version, count) => `Vaelis ${version} · アクティブセッション ${count}`,
  ...
  profiles: {
    ...
    deleting: '削除中...',
    createDesc: 'プロファイルは独立した Vaelis 環境です：設定、スキル、SOUL.md が別々になります。',
    nameLabel: '名前',
    cloneFrom: '複製元',
    cloneFromNone: 'なし（空）',
  ...
  agenda: {
    ...
    unknown: '不明',
    locationLabel: '場所',
    locationPlaceholder: '例：東2-101',
    notesLabel: 'メモ',
    notesPlaceholder: '任意',
    endBeforeStart: '終了は開始より前にできません',
    rhythmOff: 'アサイラムは未同期',
    confirm: '確認',
    dismiss: '却下',
    confirmed: '予定を確認しました',
    ...
    collect: {
      ...
      notStartedShort: '未開始',
      activeShort: '収集中',
      notStartedTitle: 'まだ収集していません',
      notStartedDesc: 'まず最近の会話を確認してください。「収集」にしない会話はインデックスに入りません（プライバシーは既定でオフ）。',
      backlogTitle: '最近の会話',
      finishReview: '確認を終えて収集を開始',
      pendingTitle: '選択待ちの新しい会話',
      pendingEmptyTitle: '新しい会話はありません',
      pendingEmptyDesc: '新しい話し相手はここに表示され、ワンクリックで収集/除外できます。',
      collect: '収集',
      exclude: '除外',
      modeKnown: '収集中',
      modeExcluded: '除外済み',
  ...
  console: {
    agentsTitle: 'エージェント',
    agentsLoading: 'エージェントを読み込み中',
    agentsLoadFailed: 'エージェントを読み込めませんでした',
    agentsEmptyTitle: 'エージェントはまだありません',
    agentsEmptyDesc: 'バックエンドが schedule secretary を登録すると、ここに表示されます。',
    newAgent: 'エージェントを追加',
    newAgentLabel: 'エージェントを追加',
    newAgentCategoryLabel: 'カテゴリ',
    newAgentFolderLabel: '作業フォルダ',
    newAgentFolderPlaceholder: 'フォルダが未選択です',
    newAgentFolderPick: 'フォルダを選ぶ',
    ...
    chat: {
      composerPlaceholder: '秘書にメッセージ…',
      send: '送信',
      cancel: '停止',
      slashHint: (command: string, description: string) => `${command} — ${description}`,
      cardInvalid: 'この提案を読み込めませんでした。',
      cardConfirmed: '確認しました。',
      cardDismissed: '無視しました。',
      cardFailed: '予定サービスに接続できません。再試行してください。'
    },
    timeline: {
      title: '予定',
      today: '今日',
      tomorrow: '明日',
      emptyTitle: '予定なし',
      emptyDesc: '秘書はまだ予定を入れていません。',
      expand: '全予定ボードを開く',
      addEvent: '予定を追加',
      addTitle: '内容',
      addTitlePlaceholder: '例：15時に歯医者',
      addStart: 'いつ',
      addSubmit: '追加',
      addCancel: 'キャンセル',
      pending: '確認待ち',
    ...
    supervisor: {
      ...
      loadFailed: 'このエージェントを読み込めませんでした。',
      retry: '再試行',
      statuses: {
        idle: '待機',
        working: '作業中',
      ...
      workLogTitle: (subagent: string) => `${subagent} — 作業ログ`,
      logLoading: '作業ログを読み込んでいます…',
      logFailed: 'この作業ログを読み込めませんでした。',
      logEmpty: 'ログはまだありません。',
      openFailed: 'その会話を開けませんでした。',
      views: {
        files: 'ファイル',
        board: 'ボード',
        artifacts: 'アーティファクト'
      ...
      chat: {
        composerPlaceholder: 'このエージェントに直接指示…',
        send: '送信',
        cancel: '停止',
        slashHint: (command: string, description: string) => `${command} — ${description}`
      },
      filesEmpty: 'プロジェクトファイルはまだありません。',
      boardEmpty: 'ボードにタスクはまだありません。',
      artifactsEmpty: 'アーティファクトはまだありません。'
    }
  },
  cron: {
    close: 'Cron を閉じる',
    ...
    newCron: '新しい Cron',
    emptyDescNew:
      'Cron 式でプロンプトを実行するスケジュールを設定します。Vaelis が実行して、選択した宛先に結果を送信します。',
  ...
  sidebar: {
    ...
    projects: {
      ...
      staleBackend:
        'プロジェクトを作成するには Vaelis バックエンドを更新してください。バックエンドがこのデスクトップアプリより古いです（設定 → 更新 → バックエンド）。',
      deleteConfirm:
        'Vaelis から保存済みプロジェクトを削除します。ファイル・git リポジトリ・ワークツリーはそのまま残ります。',
      startWork: '新しいワークツリー',
  ...
  composer: {
    ...
    placeholderStarting: 'Vaelis を起動中...',
    placeholderReconnecting: 'Vaelis に再接続中…',
    placeholderFollowUp: 'フォローアップを送信',
    secretaryL1Placeholder: '総秘書に話す…',
    secretaryL2Placeholder: name => `${name}に話す…`,
    ...
    attachUrlDesc: 'Vaelis がページを取得し、このターンのコンテキストとして含めます。',
  ...
  statusStack: {
    ...
    coding: {
      ...
      agentShip: 'Vaelis にコミットと PR を任せる',
      agentShipPrompt:
  ...
  updates: {
    ...
    unsupportedMessage: 'このバージョンの Vaelis はアプリ内から自分を更新できません。',
    ...
    availableBody: '新しいバージョンの Vaelis をインストールする準備ができています。',
    availableTitleBackend: 'バックエンドの更新があります',
    availableBodyBackend: '接続中の Vaelis バックエンドの新しいバージョンをインストールできます。',
    ...
    manualBody:
      'Vaelis をコマンドラインからインストールしたため、更新もそこで実行されます。これをターミナルに貼り付けてください:',
    manualPickedUp: 'Vaelis は次回起動時に新しいバージョンを読み込みます。',
    guiSkewTitle: 'デスクトップアプリを更新してください',
    guiSkewBody:
      'バックエンドは更新されましたが、このデスクトップアプリのパッケージは変更されていません。一致させるために Vaelis デスクトップアプリ（AppImage / .deb / .rpm）を更新または再インストールしてください。',
    copy: 'コピー',
    copied: 'コピーしました',
    done: '完了',
    applyingBody:
      'Vaelis アップデーターが独自のウィンドウで引き継ぎ、完了後に自動的に Vaelis を再度開きます。更新中はご自分で Vaelis を開き直さないでください。',
    applyingBodyBackend: 'リモートバックエンドが更新を適用して再起動します。復帰すると Vaelis が自動的に再接続します。',
    applyingClose: 'このウィンドウは更新中に閉じ、その後 Vaelis が自動的に再度開きます。',
  ...
  install: {
    ...
    oneTimeTitle: 'Vaelis には一度限りのインストールが必要です',
    ...
    settingUpTitle: 'Vaelis Agent を設定中',
    finishingTitle: '仕上げ中',
    failedDesc:
      'インストール手順のいずれかが失敗しました。Windows では、別の Vaelis CLI またはデスクトップインスタンスが実行中の場合に発生することがあります。実行中の Vaelis インスタンスをすべて停止してから再試行してください。詳細は以下またはデスクトップログで確認できます。',
    activeDesc:
      'これは一回限りのセットアップです。Vaelis インストーラーが依存関係をダウンロードしてマシンを設定しています。以降の起動ではこの手順はスキップされます。',
  ...
  onboarding: {
    headerTitle: 'Vaelis Agent のセットアップをしましょう',
    headerDesc: 'チャットを始めるにはモデルプロバイダーを接続してください。ほとんどのオプションはワンクリックです。',
    preparingInstall: 'Vaelis はインストールを完了中です。初回実行では通常 1 分以内に完了します。',
    starting: 'Vaelis を起動中…',
    ...
    recommended: '推奨',
    connected: '接続済み',
    featuredPitch: '1 つのサブスクリプションで 300 以上の最先端モデル — Vaelis を実行するための推奨方法',
    ...
    apiKeyOptions: {
      ...
      local: {
        short: 'セルフホスト',
        description:
          'ローカルまたはセルフホストの OpenAI 互換エンドポイント（vLLM、llama.cpp、Ollama など）に Vaelis を接続。'
      }
    },
    ...
    flowSubtitles: {
      ...
      device_code: 'ブラウザーで確認ページを開きます — Vaelis が自動接続します',
    ...
    authorizeThere: 'そこで Vaelis を承認してください。',
    copyAuthCode: '認証コードをコピーして以下に貼り付けてください。',
    pasteAuthCode: '認証コードを貼り付け',
    reopenAuthPage: '認証ページを再度開く',
    autoBrowser: provider =>
      `${provider} をブラウザーで開きました。Vaelis をそこで承認すれば自動接続されます。コピーや貼り付けは不要です。`,
  ...
  shell: {
    ...
    statusbar: {
      ...
      gatewayTitle: 'Vaelis 推論ゲートウェイのステータス',
      agents: 'エージェント',
  ...
  preview: {
    ...
    largeBody: (label, size) => `${label} は ${size} です。Vaelis は最初の 512 KB のみを表示します。`,
    ...
    web: {
      ...
      tryAgain: '再試行',
      restarting: 'Vaelis を再起動中...',
      askRestart: 'Vaelis にサーバーの再起動を依頼',
      lookingRestart: taskId => `Vaelis は再起動するプレビューサーバーを検索中です (${taskId})`,
      restartingTitle: 'プレビューサーバーを再起動中',
      restartingMessage: 'Vaelis はバックグラウンドで作業中です。進捗はプレビューコンソールで確認してください。',
      ...
      finishedRestarting: message =>
        `Vaelis がプレビューサーバーの再起動を完了しました${message ? `: ${message}` : ''}`,
      ...
      restartFailedMessage: 'Vaelis がサーバーを再起動できませんでした。',
      stillWorking:
        'Vaelis はまだ作業中ですが、再起動の結果がまだ届いていません。サーバーコマンドがフォアグラウンドで実行されている可能性があります。',
  ...
  assistant: {
    thread: {
      ...
      loadingResponse: 'Vaelis が応答を読み込み中',
    ...
    approval: {
      gatewayDisconnected: 'Vaelis ゲートウェイが接続されていません',
      ...
      alwaysDescription: pattern =>
        `これにより "${pattern}" パターンが永続的な許可リスト (~/.hermes/config.yaml) に追加されます。Vaelis はこのセッションや将来のセッションで、このようなコマンドについて再度尋ねません。`,
      alwaysAllow: '常に許可'
    },
    clarify: {
    ...
    tool: {
      ...
      mutate: {
        ...
        pending: '予定を記録中',
        cancelMatching: {
          title: count => `時間割を取り消しました · ${count} 件`,
          pending: '時間割を取り消し中',
          fromDate: date => `${date} 以降`,
          empty: '一致する今後の授業はありません',
          more: count => `+${count}`
        }
      },
      query: {
      ...
      project: {
        title: count => `プロジェクト · ${count}`,
        viewing: 'プロジェクトを確認中',
        hours: hours => `週 ${hours}h`,
        noRhythm: 'ペース未設定',
        empty: '進行中のプロジェクトはありません'
      },
      planDay: {
        pending: '予定を立てています',
        title: (date, count) => `${date} を計画 · ${count} 件`,
        empty: 'この日は予定が入りません',
        conflicts: count => `競合 ${count}`
      },
      checkin: {
        pending: '好みを記録中',
        title: count => `確認待ち · 確認 ${count}`,
        empty: '今回は質問はありません'
      },
      actions: {
        read: '読み取り完了',
  ...
  prompts: {
    ...
    sudoTitle: '管理者パスワード',
    sudoDesc:
      'Vaelis は特権コマンドを実行するために sudo パスワードが必要です。ローカルエージェントにのみ送信されます。',
    sudoPlaceholder: 'sudo パスワード',
    secretTitle: 'シークレットが必要です',
    secretDesc: 'Vaelis は続行するための認証情報が必要です。',
  ...
  desktop: {
    ...
    restartToUseSaveImage: '画像を保存するには Vaelis Desktop を再起動してください。',
    restartToSaveImages: '画像を保存するには Vaelis Desktop を再起動してください',
