...
describe('mergeSessionPage', () => {
  ...
  it('dedupes two rows with the same live id in the same incoming page', () => {
    // The backend can return the same session twice in one response (cross-
    // profile UNION, concurrent-write race, or a pre-existing duplicated
    // state.db row). Without an incoming-level guard the dup renders as a
    // second sidebar entry that survives a hard restart.
    const incoming = [
      session({ id: 'dup', title: 'first copy' }),
      session({ id: 'dup', title: 'second copy (freshest)' })
    ] as SessionInfo[]

    const merged = mergeSessionPage([], incoming, [])

    expect(merged.map(s => s.id)).toEqual(['dup'])
    // Later row wins on collision so the freshest title/preview survives.
    expect(merged[0]?.title).toBe('second copy (freshest)')
  })

  it('dedupes two rows that share a lineage root in the same incoming page', () => {
    // Same scenario as above, but for a compressed lineage where both the
    // live tip and a duplicated copy share _lineage_root_id.
    const incoming = [
      session({ id: 'tip-1', _lineage_root_id: 'root', title: 'old' }),
      session({ id: 'tip-2', _lineage_root_id: 'root', title: 'new' })
    ] as SessionInfo[]

    const merged = mergeSessionPage([], incoming, [])

    expect(merged.map(s => s.id)).toEqual(['tip-2'])
