...
export function mergeSessionPage(
  ...
  // Dedupe `incoming` by lineage FIRST. The backend can return the same
  // session twice in a single page (cross-profile UNION in listSessions,
  // concurrent-write race during turn persist, or a duplicated state.db row
  // from before this guard). Without this step the dup leaks straight into
  // `$sessions` and renders as two identical sidebar rows that survive a
  // hard restart (the dup row is already persisted). Later row wins on key
  // collision (it carries the freshest title / preview / cwd).
  const dedupedIncoming = new Map<string, SessionInfo>()

  for (const session of incoming) {
    dedupedIncoming.set(session._lineage_root_id ?? session.id, session)
  }

  const dedupedIncomingList = [...dedupedIncoming.values()]
  ...
  const merged = dedupedIncomingList.map(session => {
