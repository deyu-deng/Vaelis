...
import { textPart } from './chat-messages'
import {
...
describe('toBranchableMessageRepository', () => {
  it('chains user → assistant so the thread head is not an orphan last reply', () => {
    const user = { id: 'u1', role: 'user' as const, parts: [textPart('hi')] }
    const assistant = { id: 'a1', role: 'assistant' as const, parts: [textPart('hello')] }

    const repo = toBranchableMessageRepository([user, assistant], {
      runtimeMessageCache: new WeakMap(),
      toolMergeCache: createToolMergeCache()
    })

    const byId = new Map(repo.messages.map(item => [item.message.id, item]))
  ...
  it('does not flatten every row to parentId null (that hides all but the last reply)', () => {
    const messages = [
      { id: 'u1', role: 'user' as const, parts: [textPart('one')] },
      { id: 'a1', role: 'assistant' as const, parts: [textPart('two')] },
      { id: 'u2', role: 'user' as const, parts: [textPart('three')] },
      { id: 'a2', role: 'assistant' as const, parts: [textPart('four')] }
    ]

    const repo = toBranchableMessageRepository(messages, {
      runtimeMessageCache: new WeakMap(),
      toolMergeCache: createToolMergeCache()
    })

    const parentIds = repo.messages.map(item => item.parentId)

    expect(parentIds.filter(id => id === null)).toHaveLength(1)
