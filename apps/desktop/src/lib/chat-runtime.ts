 ...
 *
 * Only the leading status prefix (e.g. "Vaelis is thinking...") and the
...
export function quickModelOptions(
  ...
  // Merge connected desktop-quota apps (Antigravity, …) so their models appear
  // in the composer quick-switcher alongside backend providers.
  const providers = [...(data?.providers ?? []), ...desktopQuotaProviders()].sort((a, b) => {
...
/** Chain `$messages` into an assistant-ui branchable repo.
 *
 *  Every visible message's parent is the previous visible message. Putting
 *  `parentId: null` on every row (and `headId` on the last) makes each turn a
 *  separate root — the thread then only renders the head, i.e. the last
 *  assistant reply. ChatView and ChatSurface must share this helper. */
export function toBranchableMessageRepository(
  messages: ChatMessage[],
  caches: {
    runtimeMessageCache: WeakMap<ChatMessage, ThreadMessage>
    toolMergeCache: ToolMergeCache
  }
) {
  ...
  for (const message of coalesceToolOnlyAssistants(messages, caches.toolMergeCache)) {
    ...
    const cachedMessage = caches.runtimeMessageCache.get(message)
    const runtimeMessage = cachedMessage ?? toRuntimeMessage(message)

    if (!cachedMessage) {
      caches.runtimeMessageCache.set(message, runtimeMessage)
    }
