...
export interface AgendaEvidence {
  ...
  talker_name?: string
  /** Venue / room mentioned in the message — distinct from `AgendaEvent.location`
   *  which is the explicitly-typed field on the row. */
  location?: string
}
...
export interface AgendaEvent {
  ...
  id: string
  kind: AgendaKind
  /** WP-DAY-FACE: optional venue / room — flat field the backend exposes on the row. */
  location?: null | string
  /** WP-DAY-FACE: free-form notes the user typed (manual events). */
  notes?: null | string
...
export interface AgendaEventCreate {
  end_at?: string
  kind?: AgendaKind
  /** WP-DAY-FACE: optional venue. The server treats null / missing identically. */
  location?: null | string
  /** WP-DAY-FACE: optional free-form notes. */
  notes?: null | string
  start_at: string
  title: string
}
...
/**
 * WP-DAY-FACE: anchors are the day's routine blocks (sleep / meals) the
 * backend surfaces on `/api/agenda/day`. They sit on the same axis as events
 * with a distinct colour, never block-conflict, and are NOT actionable.
 */
export interface AgendaAnchor {
  /** Free-form label; rendered verbatim. */
  label: string
  /** Inclusive start of the anchor (naive local ISO). */
  start_at: string
  /** Inclusive end of the anchor (naive local ISO). Anchors always span. */
  end_at: string
}

/** A plan item shown on the day axis alongside events / anchors. */
export interface AgendaPlanItem {
  end_at: null | string
  /** Plan items always render in the same anchor lane; never become pending. */
  id: string
  notes?: null | string
  /** `anchor` / `event` / `plan` — always present in API output. */
  lane: 'anchor' | 'event' | 'plan'
  start_at: string
  title: string
}

/** Wire shape for `GET /api/agenda/day?date=YYYY-MM-DD`. */
export interface AgendaDay {
  anchors: AgendaAnchor[]
  /**
   * Optional confirmed avoid-windows (e.g. "Lunchtime 12:00–13:00", "Focus block").
   * Drawn full-width as a faint, untitled, pointer-events:none band behind the
   * regular blocks. Older / older-shape responses omit the field → the renderer
   * simply skips it (裁定 39 / WP-AXIS-TODAY). Naive local ISO; empty array ==
   * no bands, also rendered as nothing.
   */
  avoid_windows?: AgendaAvoidWindow[]
  date: string
  events: AgendaEvent[]
  plan_items: AgendaPlanItem[]
}

/** Confirmed slot the user has reserved (renderer skips rendering collisions). */
export interface AgendaAvoidWindow {
  end_at: string
  start_at: string
}
