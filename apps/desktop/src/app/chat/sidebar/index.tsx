...
const NAV_ICON = 'size-4 shrink-0 text-[color-mix(in_srgb,currentColor_72%,transparent)]'
...
export function ChatSidebar({
  ...
  // Secretary route nav: L1 swaps new-session for new-agent; L2 hides the
  // global Skills entry (it inherits from L1) and adds "back to secretary".
  const navItems = useMemo(() => {
    if (l1) {
      return SIDEBAR_NAV.filter(item => item.id !== 'new-session')
    }

    if (l2Id) {
      return SIDEBAR_NAV.filter(item => item.id !== 'skills')
    }
  ...
  return (
    ...
    >
      <SidebarContent className="gap-0 overflow-hidden bg-transparent px-2.5">
        ...
        {contentVisible && showSessionSections && !l1 && (
        ...
        {contentVisible && !showSessionSections && !l1 && (
          <SidebarBlankState
            actionLabel={level ? s.nav['new-session'] : s.projects.newButton}
            onAction={level ? () => onNewSessionInWorkspace(null) : openProjectCreate}
          />
        )}
