...
import { abortDesktopQuota } from '@/lib/desktop-quota-chat';
...
export function usePromptActions({ activeSessionId, activeSessionIdRef, busyRef, branchCurrentSession, createBackendSessionForSend, desktopQuotaStream, getRouteToken, handleSkinCommand, openMemoryGraph, refreshSessions, requestGateway, resumeStoredSession, selectedStoredSessionIdRef, startFreshSessionDraft, sttEnabled, updateSessionState }) {
    const { t } = useI18n();
    ...
    const cancelRun = useCallback(async () => {
        ...
        // Abort an in-flight desktop-quota (aigw) stream for this session, if any.
        // The backend's session.interrupt below is a no-op for those turns (they
        // never called prompt.submit), so the local AbortController is the only
        // way to stop the streaming fetch.
        abortDesktopQuota(sessionId);
        try {
