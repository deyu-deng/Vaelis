...
// Stream the turn through aigw without hitting the network — we only assert the
// routing decision (model id, base url, the fact that prompt.submit is skipped).
vi.mock('@/lib/desktop-quota-chat', async (importOriginal) => {
    const actual = await importOriginal();
    return {
        ...actual,
        streamDesktopQuotaChat: vi.fn(async () => { })
    };
});
...
describe('usePromptActions desktop-quota routing (Local Hub / aigw)', () => {
    beforeEach(() => {
        ...
        $currentProvider.set('antigravity');
        $currentModel.set('gemini-3-pro');
        $messages.set([]);
    });
    afterEach(() => {
        $connectedDesktopApps.set({});
        $currentProvider.set('');
        $currentModel.set('');
        vi.mocked(streamDesktopQuotaChat).mockClear();
    });
    it('routes an Antigravity-selected send to aigw (not prompt.submit), with the joined model id', async () => {
        const requestGateway = vi.fn(async () => ({}));
        let handle = null;
        render(_jsx(Harness, { activeSessionId: RUNTIME_SESSION_ID, onReady: h => (handle = h), refreshSessions: vi.fn(async () => undefined), requestGateway: requestGateway }));
        const ok = await handle.submitText('hello from the desktop quota');
        expect(ok).toBe(true);
        ...
        expect(requestGateway).not.toHaveBeenCalledWith('prompt.submit', expect.anything());
        const req = vi.mocked(streamDesktopQuotaChat).mock.calls[0][0];
    ...
    it('does not route a normal backend provider through aigw', async () => {
        $connectedDesktopApps.set({});
        $currentProvider.set('openai');
        $currentModel.set('gpt-4o');
        expect(isDesktopQuotaProvider($currentProvider.get())).toBe(false);
        const requestGateway = vi.fn(async (method) => (method === 'prompt.submit' ? {} : {}));
        let handle = null;
        render(_jsx(Harness, { activeSessionId: RUNTIME_SESSION_ID, onReady: h => (handle = h), refreshSessions: vi.fn(async () => undefined), requestGateway: requestGateway }));
