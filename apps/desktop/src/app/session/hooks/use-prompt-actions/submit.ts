...
import {
  buildDesktopQuotaMessages,
  type DesktopQuotaStreamHandlers,
...
interface SubmitPromptDeps {
  ...
  desktopQuotaStream: DesktopQuotaStreamHandlers
...
export function useSubmitPrompt(deps: SubmitPromptDeps) {
  ...
  return useCallback(
    async (rawText: string, options?: SubmitTextOptions) => {
      ...
      // Re-baselined after a successful session.create below (the minted session
      // IS this submit's target), so it cannot be const.
      ...
      if (!sessionId) {
        ...
        // The session just minted above IS this submit's target, so re-baseline
        // the drift guard on it instead of aborting on the very session we
        // created. `createBackendSessionForSend` already closes the session and
        // returns null when the context really drifted (user switched away), so
        // a non-null id means it is ours. Without this the ref moving from the
        // fresh-draft value to the new stored id read as "drift" and the turn
        // was dropped silently — no prompt.submit, and because the backend
        // writes the session row lazily on first prompt, no session anywhere
        // (L2 /agent/:id first-message bug).
