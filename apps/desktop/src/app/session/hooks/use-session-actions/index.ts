  ...
  const createBackendSessionForSend = useCallback(
    async (preview: string | null = null): Promise<string | null> => {
      ...
      try {
        ...
        // as the transcript container + sidebar row); the model override lives
        // only in renderer state ($currentModel/$currentProvider).
        const isDesktopQuota = isDesktopQuotaProvider(uiProvider)
        ...
        const created = await requestGateway<SessionCreateResponse>('session.create', {
          ...
          ...(!isDesktopQuota && uiModel ? { model: uiModel, ...(uiProvider ? { provider: uiProvider } : {}) } : {}),
          ...(!isDesktopQuota && uiEffort ? { reasoning_effort: uiEffort } : {}),
          ...(!isDesktopQuota && uiFast ? { fast: true } : {})
        })
        ...
        if (stored) {
          ...
          // L1/L2 secretary shells must not jump to `/:sessionId` — that route no
          // longer hosts chat (WP-E), so shellLevel goes null and the center
          // bounces home mid-send (L2 first-message bug).
          if (shouldSyncSessionUrl(pathnameFromRouteToken(startingRouteToken))) {
  ...
  const forkBranch = useCallback(
    async (branchMessages: BranchMessage[], parentStoredId: null | string, cwd?: string): Promise<boolean> => {
      ...
      try {
        ...
        if (shouldSyncSessionUrl(pathnameFromRouteToken(getRouteToken()))) {
