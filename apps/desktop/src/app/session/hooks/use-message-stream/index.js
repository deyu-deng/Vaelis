...
export function useMessageStream({ activeSessionIdRef, hydrateFromStoredSession, queryClient, refreshHermesConfig, refreshSessions, sessionStateByRuntimeIdRef, updateSessionState }) {
    ...
    const failAssistantMessage = useCallback((sessionId, errorMessage) => {
        updateSessionState(sessionId, state => {
            ...
            const groupId = state.pendingBranchGroup ?? undefined;
            const prev = state.messages;
            const error = errorMessage.trim() || 'Vaelis reported an error';
    ...
    // failAssistantMessage is exposed so the desktop-quota send path (which
    // bypasses the gateway event stream) can finalize an aigw stream failure
    // through the same state mutation the gateway event handler uses.
    return {
        appendAssistantDelta,
        appendReasoningDelta,
        completeAssistantMessage,
