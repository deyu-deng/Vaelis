...
describe('createBackendSessionForSend secretary shell URL (WP-L2-FE-BUGFIX)', () => {
  afterEach(() => {
    cleanup()
    $newChatProfile.set(null)
    $activeGatewayProfile.set('default')
  ...
  async function createOnRoute(routeToken: string) {
    const navigate = vi.fn()
    let create: ((preview?: string | null) => Promise<string | null>) | null = null

    const requestGateway = vi.fn(async (method: string) => {
      if (method === 'session.create') {
        return { session_id: RUNTIME_SESSION_ID, stored_session_id: 'stored-new-1' } as never
      }
  ...
  it('stays on /agent/:id after the first session.create (no bounce home)', async () => {
    const { navigate, sessionId } = await createOnRoute('/agent/simulation::')

    expect(sessionId).toBe(RUNTIME_SESSION_ID)
    expect(navigate).not.toHaveBeenCalled()
  })

  it('stays on L1 / after session.create', async () => {
    const { navigate, sessionId } = await createOnRoute('/::')

    expect(sessionId).toBe(RUNTIME_SESSION_ID)
    expect(navigate).not.toHaveBeenCalled()
  })

  it('still rewrites /new to the stored session route', async () => {
    const { navigate, sessionId } = await createOnRoute('/new::')

    expect(sessionId).toBe(RUNTIME_SESSION_ID)
    expect(navigate).toHaveBeenCalledWith('/stored-new-1', { replace: true })
  })
})
